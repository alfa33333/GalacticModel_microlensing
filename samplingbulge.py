...
def prior_transform_bulge(unif_sphere):
    ...
    gal_prior[0] = np.interp(unif_sphere[0],bulgemasscdf,xpdf)  # Sample from the uniform sphere to the mass.
    gal_prior[1] = np.interp(unif_sphere[1],bulgemassdencdf,xpdfsource) # Sample from the uniform sphere to the bulge sources.
    _, invbulge, _ , kbulge, kdisc = invdensitycdf(unif_sphere[2],gal_prior[1])  # lenses density, it needs the  source value
    k_tot = kbulge + kdisc
    k_bulge = kbulge/k_tot
    gal_prior[2] = invbulge 
    gal_prior[3] = invcdfvel(unif_sphere[3],gal_prior[2], gal_prior[1],'bulge')  # velocity, it needs source and lens positions.
    gal_prior[4] = kbulge
    gal_prior[5] = k_tot
    return gal_prior

def loglike_bulge_gal(sampled_val):
    re0c = galfunc.re0_ds(sampled_val[1]*3.086e16)
...
def improved_run_bulge(dlogz=0.1, ndim=6,npdim=4):
    """ simple main function for sampling. """
    # initialize our nested sampler
    print('Starting sampling')
    cpunum = cpu_count() 
    with Pool(cpunum-1) as executor:
        sampler = dynesty.NestedSampler(loglike_bulge_gal,prior_transform_bulge,ndim=ndim,npdim=npdim,\
                pool=executor,
                queue_size = cpunum,
                bootstrap=0)
        ...
        res = sampler.results
        res.summary()
        save_obj(res, 'sanity_check_bulge3')
        #return res
